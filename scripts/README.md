...
| Script | Purpose |
|---|---|
| `render_projections.sh` | Render X-ray projections from Kelvin lattice YAML files via the Go renderer. Two-stage per timestep: YAML → voxel grid, then voxel grid → PNG projections. Reads YAMLs from `data/kelvin/yaml/`, writes renders to `data/kelvin/renders/`. |
...
| Script | Purpose |
|---|---|
| `train_kelvin_workshop.sh` | Run the full 5-stage NeRF training sequence on the Kelvin dataset: canonical forward → canonical backward → velocity field res-6 → velocity field res-12 → spatiotemporal mix. Outputs checkpoints to `outputs/kelvin/`. |
| `eval_kelvin.py` | Evaluate trained models: renders x-z density cross-sections at selected timesteps for all three model stages plus the GT FEM volume, and plots the spatiotemporal mixing coefficient α(t). Saves `eval_xsections.png` and `eval_mixing.png` to `outputs/kelvin/`. |
