...
| Show **precomputed** results (TensorBoard, GIFs, slices) if available | Full Kelvin pipeline under `data/kelvin/` |
...
| 0:20 | **Kelvin workflow:** `fem_lattice_simulator` → `data/kelvin/` → `neural_xray` |
...
Details: [`data/kelvin/README.md`](../data/kelvin/README.md)
...
See [SETUP_COLAB.md](SETUP_COLAB.md). `KeyError: 'optimizers'`: delete `outputs/kelvin/xray_vfield/` and re-run training.
